...
//     await apiCall(
//       async () => {
//         const res = await apiClient.get<{ rooms: Room[] }>(
...
//             data: { guest: Guest; invoice: Invoice | null };
...
//             `/api/guests/check-out-Guest/${id}/checkout`,
//             {}
//           );
//         },
//         async () => {
...
export const GuestProvider = ({ children }: { children: ReactNode }) => {
  ...
  const fetchGuests = useCallback(async () => {
    await apiCall(
      () => apiClient.get<{ guests: Guest[] }>("/api/guests/get-all-guest").then(res => res.data.guests),
      (data) => setGuests(data),
      "Failed to fetch guests"
    );
  }, [apiCall]);
  ...
  const fetchGuestsByCategory = useCallback(async (category: string) => {
    await apiCall(
      () => apiClient.get<{ data: Guest[] }>(`/api/guests/get-guest-by-category?category=${encodeURIComponent(category)}`).then(res => res.data.data),
      (data) => setGuests(data),
      `Failed to fetch guests in category: ${category}`
    );
  }, [apiCall]);
  ...
  const fetchGuestById = useCallback(async (id: string) => {
    await apiCall(
      () => apiClient.get<{ data: { guest: Guest; invoice: Invoice | null } }>(`/api/guests/get-Guest-By-Id/${id}`).then(res => res.data.data),
      (data) => {
        setGuest(data.guest);
        setInvoice(data.invoice);
      },
  ...
  const contextValue = useMemo(() => ({
    guests, guest, invoice, loading, error,
    fetchGuests, fetchGuestsByCategory, fetchGuestById,
    createGuest, updateGuest, checkoutGuest, deleteGuest,
    downloadInvoicePdf, sendInvoiceByEmail,
  }), [
    guests, guest, invoice, loading, error,
    fetchGuests, fetchGuestsByCategory, fetchGuestById,
    createGuest, updateGuest, checkoutGuest, deleteGuest,
  ...
  return (
    <GuestContext.Provider value={contextValue}>{children}</GuestContext.Provider>
  );
};
