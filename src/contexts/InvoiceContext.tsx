...
interface InvoiceContextType {
  ...
  updateInvoiceStatus: (id: string, status: 'pending' | 'paid' | 'cancelled') => Promise<void>;
  deleteInvoice: (id: string) => Promise<void>;
  sendInvoiceByEmail: (id: string) => Promise<{ success: boolean; message: string }>;
...
export const InvoiceProvider = ({ children }: { children: ReactNode }) => {
  ...
  const fetchAllInvoices = useCallback(async (page = 1, limit = 25) => {
    await apiCall(
      async () => {
        const res = await apiClient.get<PaginatedInvoices>(`/api/invoice/get-all-invoices?page=${page}&limit=${limit}`);
        return res.data;
      },
  ...
  const searchInvoices = useCallback(async (params: InvoiceSearchParams) => {
    const query = new URLSearchParams(params as Record<string, string>).toString();
    await apiCall(
      async () => {
        const res = await apiClient.get<{ data: Invoice[] }>(`/api/invoice/search-Invoices?${query}`);
  ...
  const fetchInvoiceById = useCallback(async (id: string) => {
    await apiCall(
      async () => {
        const res = await apiClient.get<{ data: Invoice }>(`/api/invoice/get-Invoice-By-Id/${id}`);
        return res.data.data;
      },
  ...
  const updateInvoiceStatus = useCallback(async (id: string, status: 'pending' | 'paid' | 'cancelled') => {
    await apiCall(
  ...
  }, [apiCall, fetchAllInvoices, fetchInvoiceById, paginatedInvoices, currentInvoice]);
  ...
  const sendInvoiceByEmail = useCallback(async (id: string) => {
  ...
  const downloadInvoicePdf = useCallback((id: string) => {
    // This is a direct browser action, not an async API data call
    try {
      window.open(`${API_BASE}/api/invoice/${id}/download`, '_blank')?.focus();
    } catch (e) {
      setError('Could not open the download link. Please check your pop-up blocker.');
    }
  }, []);
