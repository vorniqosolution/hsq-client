                ...
                <div className="flex flex-col md:flex-row items-stretch md:items-center p-4 gap-4 md:gap-6 min-h-[80px]">
                    ...
                    <div className="flex-1 min-w-[200px] flex flex-col justify-center">
                        ...
                        <div className="mt-1.5 flex flex-wrap gap-2">
                            ...
                            {reservation.financials && reservation.financials.totalAdvance > 0 && (
                                <Badge variant="outline" className="text-[10px] bg-green-50 text-green-700 border-green-200 h-5 px-1.5">
                                    Adv: {reservation.financials.totalAdvance.toLocaleString()}
                                </Badge>
                            )}
                            {reservation.specialRequest && (
                                <div title={reservation.specialRequest} className="group relative flex items-center justify-center h-5 w-5 bg-purple-50 rounded-full cursor-help">
                    ...
                    <div className="flex-[0.8] min-w-[180px] border-l-0 md:border-l border-slate-100 md:pl-6 py-1 flex flex-col justify-center gap-1.5">
                        <div className="flex items-center justify-between gap-4 text-sm">
                            <span className="font-bold text-slate-400 text-[10px] w-8">IN</span>
                            <span className="font-medium text-slate-700 flex-1 text-right md:text-left">
                                {format(startDate, "d MMM, EEE")}
                            </span>
                        </div>
                        <div className="flex items-center justify-between gap-4 text-sm">
                            <span className="font-bold text-slate-400 text-[10px] w-8">OUT</span>
                            <span className="font-medium text-slate-700 flex-1 text-right md:text-left">
                    ...
                    <div className="flex-[0.6] min-w-[140px] border-l-0 md:border-l border-slate-100 md:pl-6 py-1 flex flex-col justify-center">
                        ...
                        <p className="text-xs text-slate-500 mt-1">
                            {reservation.adults || 1} Adult{(reservation.adults || 1) !== 1 ? 's' : ''}
                            {(reservation.infants || 0) > 0 && `, ${reservation.infants} Infant${reservation.infants !== 1 ? 's' : ''}`}
                        </p>
                    </div>
                    ...
                    <div className="flex-[0.8] border-l-0 md:border-l border-slate-100 md:pl-6 py-1 flex items-center justify-between md:justify-end gap-3 md:gap-6">
                        ...
                        <div className="flex items-center gap-2 mr-auto md:mr-0">
                            {reservation.status === 'checked-in' ? (
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-sm font-medium text-sky-600">Reserved</span>
                                </div>
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-sm font-medium text-slate-500 capitalize">{reservation.status}</span>
                                </div>
                            )}
                        </div>
                        ...
                        <div className="flex items-center gap-1">
                            ...
                            {(status === "reserved" || status === "upcoming" || status === "confirmed") && (
                                <Button
                                    variant="ghost"
                                    size="icon"
                                    onClick={onCheckIn}
                            ...
                            {onSwap && (status === "reserved" || status === "confirmed" || status === "upcoming") && (
                                <Button
                                    variant="ghost"
                                    size="icon"
                                    onClick={onSwap}
