...
const DashboardPage = () => {
  ...
  const timelineQuery = useQuery({
    ...
    queryFn: async () => {
      ...
      const res = await apiClient.get<{ timeline: TimelineEvent[] }>(
        `/api/rooms/${selectedRoomId}/timeline`,
      );
  ...
  const cancelReservationMutation = useMutation({
    mutationFn: (reservationId: string) =>
      apiClient.delete(
        `/api/reservation/cancel-reservation/${reservationId}/cancel`,
      ),
    onSuccess: () => {
    ...
    return (
      ...
      >
        <div className="flex items-center gap-4 z-10">
          <div
            className={`p-3 rounded-xl bg-white shadow-sm ring-1 ring-inset transition-colors duration-300 group-hover:bg-white/80 ${colorClass.replace("text-", "ring-").replace("600", "100")}`}
          >
                ...
                <CardContent className="p-0 flex-1 flex flex-col">
                  ...
                  >
                    <div className="px-6 py-3 border-b bg-slate-50/50">
                      <TabsList className="bg-slate-200/60 h-auto p-1 flex-wrap justify-start gap-1">
                        <TabsTrigger value="All" className="text-xs px-3 h-7">
                          All Rooms
                        </TabsTrigger>
                        <TabsTrigger
                          value="Available"
                          className="text-xs px-3 h-7 text-emerald-700 data-[state=active]:bg-white data-[state=active]:text-emerald-800"
                        >
                          Available
                        </TabsTrigger>
                        <TabsTrigger
                          value="Occupied"
                          className="text-xs px-3 h-7 text-amber-700 data-[state=active]:bg-white data-[state=active]:text-amber-800"
                        >
                          Occupied
                        </TabsTrigger>
                        <TabsTrigger
                          value="Arrival"
                          className="text-xs px-3 h-7 text-purple-700 data-[state=active]:bg-white data-[state=active]:text-purple-800"
                        >
                          Arrivals
                        </TabsTrigger>
                        <TabsTrigger
                          value="Reserved"
                          className="text-xs px-3 h-7 text-sky-700 data-[state=active]:bg-white data-[state=active]:text-sky-800"
                        >
                          Reserved
                        </TabsTrigger>
                        <TabsTrigger
                          value="Maintenance"
                          className="text-xs px-3 h-7 text-red-700 data-[state=active]:bg-white data-[state=active]:text-red-800"
                        >
                          Maintenance
                        </TabsTrigger>
                      </TabsList>
                            ...
                            return (
                              ...
                              >
                                <Tooltip>
                                  ...
                                  <TooltipContent
                                    className="p-3 bg-slate-900 text-slate-50 border-0 shadow-xl rounded-lg text-xs"
                                    side="top"
                                  >
                                    ...
                                    <div className="space-y-0.5 opacity-90">
                                      ...
                                      {room.details?.guestName && (
                                        <p className="text-amber-300 mt-1 pb-0.5 border-b border-white/20">
                                          Guest: {room.details.guestName}
                                        </p>
                                      )}
                                      {room.details?.checkOut && (
                                        <p>
                                          Out:{" "}
                                          {format(
                                            parseISO(room.details.checkOut),
                                            "MMM d, h:mm a",
                                          )}
                                        </p>
                                      )}
                  ...
                  {isLoading ? (
                    ...
                    <div className="flex flex-col items-center justify-center py-12 text-center px-4">
                      ...
                      <p className="text-xs text-slate-400 mt-1">
                        Check reservations for upcoming bookings
                      </p>
                    </div>
                  ) : (
                    <div className="divide-y divide-slate-100 max-h-[500px] overflow-y-auto">
                      {arrivals.map((guest) => (
                        <div
                          key={guest._id}
                          className="p-4 hover:bg-purple-50/30 transition-colors flex items-start gap-4 group cursor-pointer"
                        >
                            ...
                            <div className="flex items-center gap-2 mt-1.5 text-xs text-slate-500">
                              ...
                              <span className="flex items-center gap-1">
                                ...
                                {guest.expectedArrivalTime ||
                                  format(parseISO(guest.startAt), "MMM d")}
                              </span>
                            </div>
                          </div>
                          <Button
                            size="icon"
                            variant="ghost"
                            className="h-8 w-8 text-slate-400 group-hover:text-purple-600"
                          >
        ...
        <DialogContent className="max-w-md p-0 overflow-hidden bg-white border-0 shadow-2xl rounded-2xl">
          <DialogHeader className="px-6 py-5 bg-slate-50 border-b">
            ...
            <DialogDescription className="text-slate-500">
              Timeline for Room{" "}
              {roomStatuses.find((r) => r._id === selectedRoomId)?.roomNumber}
            </DialogDescription>
          </DialogHeader>
            ...
            {timelineQuery.isLoading ? (
              ...
              <div className="text-center py-12 flex flex-col items-center">
                ...
                <p className="text-slate-500 text-sm mt-1 max-w-[200px]">
                  No upcoming bookings or active guests for the next 30 days.
                </p>
                <Button
              ...
              <div className="relative pl-4 space-y-0 text-sm">
                ...
                {roomTimeline.map((event, idx) => {
                  ...
                  const isCheckedInGuest =
                    event.type.includes("Guest") &&
                  ...
                  return (
                    <div
                      key={idx}
                      className="relative z-10 flex gap-4 pb-8 last:pb-0 group"
                    >
                      {/* Icon */}
                      <div
                        className={`flex-shrink-0 h-8 w-8 rounded-full flex items-center justify-center ring-4 ring-white shadow-sm ${
                          isAvailable
                            ? isNow
                      ...
                      {/* Content Card */}
                      <div
                        className={`flex-1 rounded-xl border p-4 transition-all duration-200 hover:shadow-md ${
                          isAvailable
                            ? isNow
                      ...
                      >
                        <div className="flex justify-between items-start mb-2">
                          <div>
                            <span
                              className={`font-bold ${isAvailable ? "text-emerald-700" : "text-slate-900"}`}
                            >
                              {event.name}
                            </span>
                            {isNow && (
                              <span className="ml-2 text-[10px] bg-emerald-600 text-white px-1.5 py-0.5 rounded uppercase font-bold tracking-wider">
                                Now
                              </span>
                            )}
                          </div>
                        ...
                        {canCancel && event.id && (
                          <div className="mt-3 pt-3 border-t border-slate-100 flex justify-end">
                            ...
                            >
                              ...
                              cancelReservationMutation.variables === event.id
                                ? "Cancelling..."
                                : "Cancel Reservation"}
                            </Button>
                          </div>
