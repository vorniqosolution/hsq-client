...
const InvoicesPage = () => {
  ...
  const allInvoices = useMemo(() => {
    const source =
      searchQuery.trim() !== "" ? searchResults : paginatedInvoices?.data || [];
    if (statusFilter === "all") {
      return source;
    }
  ...
  const pendingCount = totalInvoices.filter(
    (inv) => inv.status === "pending"
  ).length;
  const cancelledCount = totalInvoices.filter(
    (inv) => inv.status === "cancelled"
  ).length;

  const paidAmount = totalInvoices
              ...
              <Card className="border-0 shadow-lg bg-white">
                <CardContent className="p-6">
                  ...
                  <p className="text-sm text-slate-500 mt-4">
                    {allInvoices.length} invoices in current view
                  </p>
                </CardContent>
              </Card>
              <Card className="border-0 shadow-lg bg-white">
                <CardContent className="p-6">
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <p className="text-3xl font-light mt-2 text-emerald-600">
                        {paidCount}
                      </p>
                    </div>
                ...
                <CardContent className="p-6">
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <p className="text-3xl font-light mt-2 text-amber-600">
                        {pendingCount}
                      </p>
                    </div>
                          ...
                          <td className="py-4 px-6 text-right">
                            <DropdownMenu>
                              ...
                              <DropdownMenuContent align="end">
                                ...
                                <DropdownMenuItem
                                  onSelect={() => handleSendEmail(invoice._id)}
                                  className="cursor-pointer"
                                >
                                ...
                                <DropdownMenuSeparator />
                                <DropdownMenuItem
                                  onSelect={() => handleDelete(invoice._id)}
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <div className="flex space-x-2">
                      <Button
                        ...
                        size="sm"
                        onClick={() =>
                          fetchAllInvoices(paginatedInvoices.currentPage - 1)
                        }
                        disabled={
                          loading || paginatedInvoices?.currentPage === 1
                        }
                      >
                        Previous
                      </Button>
