...
//               <Button onClick={handleOpenCheckInDialog} className="bg-amber-500 hover:bg-amber-600">
...
//       if (name === "stayDuration") {
//         const n = input.valueAsNumber; // safer than Number(value)
//         setFormData((prev) => ({
//           ...prev,
...
//       if (name === "additionaldiscount") {
//         const n = input.valueAsNumber; // NaN when the field is cleared
//         setFormData((prev) => ({
//           ...prev,
//           // store 0 when empty; UI will render "" so the zero is removable
...
const GuestsPage: React.FC = () => {
  ...
  const renderContent = () => {
    ...
    return (
      <div className="space-y-4">
        {filteredGuests.map((guest) => (
          <GuestCard key={guest._id} guest={guest} onDelete={() => handleGuestDelete(guest)} />
        ))}
      </div>
    );
  };
...
const CheckInFormDialog: React.FC<CheckInFormDialogProps> = ({
    ...
    (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      setFormData(
        (prev) =>
          ({
            ...prev,
            [name]: type === "checkbox" ? (input as any).checked : input.value,
          } as unknown as CreateGuestInput)
      );
    },
    []
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (selected?.status === "reserved" && !("reservationId" in formData && (formData as any).reservationId)) {
      toast({
        title: "Reserved room",
        description:
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto sm:max-h-[80vh] h-auto">
        ...
        {prefilledRoom && prefilledRoom.status !== "available" && (
          <div className="text-sm mb-2 rounded border border-amber-300 bg-amber-50 p-2 text-amber-700">
            Room {prefilledRoom.roomNumber} is currently <b>{prefilledRoom.status}</b>. Proceed
            only if this room is reserved for this guest; otherwise pick another available room.
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-4 pt-4 ">
              ...
              <SelectContent>
                {selectableRooms.length === 0 ? (
                  rooms.length === 0 ? (
                    ...
                    <div className="px-2 py-4 text-center text-gray-500">No rooms available</div>
                  )
                ) : (
                  selectableRooms.map((r) => {
                    const isReserved = r.status === "reserved";
                    const disableReserved = isReserved && !(formData as any).reservationId;
          ...
          <div className="space-y-2">
            <Label htmlFor="additionaldiscount">Additional Discount Amount (Rs)</Label>
            <Input
              id="additionaldiscount"
              name="additionaldiscount"
              type="number"
              ...
              step="1"
              value={
                formData.additionaldiscount === 0 ? "" : String(formData.additionaldiscount)
              }
              onChange={handleFormChange}
              placeholder="1000, 2000"
              disabled={isSubmitting}
