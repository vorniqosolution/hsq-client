...
// CHANGED: The initial form state now uses dates instead of duration
const INITIAL_FORM_STATE: CreateGuestInput = {
  fullName: "",
  address: "",
  phone: "",
  ...
  email: "",
  roomNumber: "",
  checkInDate: format(new Date(), 'yyyy-MM-dd'), // Defaults to today
  checkOutDate: "",
  paymentMethod: "cash",
  applyDiscount: false,
  additionaldiscount: 0,
...
const GuestsPage: React.FC = () => {
  ...
  const [searchParams] = useSearchParams();
  const { getReservationById } = useReservationContext();
  const [prefill, setPrefill] = useState<Partial<CreateGuestInput> | null>(null);

  // NEW: Updated useEffect to pre-fill the form correctly from a reservation
  useEffect(() => {
    const qpId = searchParams.get("reservation");
    if (qpId) {
      (async () => {
        try {
          const r = (await getReservationById(qpId)) as any;
          if (r) {
            // Convert reservation data to the new form structure
            setPrefill({
              fullName: r.fullName || "",
              address: r.address || "",
              phone: r.phone || "",
              cnic: r.cnic || "",
              email: r.email || "",
              roomNumber: typeof r.room === 'object' ? r.room.roomNumber : r.roomNumber,
              // Enforce that the check-in date is always today
      ...
      <div className={`flex-1 ${isAdmin ? "lg:ml-0" : ""}`}>
        ...
        <div className="container mx-auto p-4 md:p-6 lg:p-8">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6">
            <h1 className="text-3xl font-bold tracking-tight">Guests</h1>
            <Button onClick={() => setIsCheckInDialogOpen(true)} className="bg-amber-500 hover:bg-amber-600">
              <UserPlus className="mr-2 h-4 w-4" />
              Check In Guest
            </Button>
          </div>
...
const GuestCard: React.FC<{ guest: Guest; onDelete: () => void; }> = React.memo(({ guest, onDelete }) => {
  const getStatusColor = (status: string) => status === "checked-in" ? "bg-green-100 text-green-800" : "bg-gray-100 text-gray-800";
  return (
    <Card className="hover:shadow transition-shadow">
      <CardContent className="grid grid-cols-1 md:grid-cols-4 items-center gap-4 p-4">
        ...
        <div className="flex justify-start md:justify-end items-center gap-2">
          <Link to={`/guests/${guest._id}`}><Button variant="outline" size="sm"><Eye className="mr-2 h-4 w-4" /> Details</Button></Link>
...
const GuestListSkeleton: React.FC = () => (
  <div className="space-y-4">{[...Array(3)].map((_, i) => <Card key={i}><CardContent className="p-4"><Skeleton className="h-24 w-full" /></CardContent></Card>)}</div>
);

// NEW: This is the completely rewritten CheckInFormDialog component
interface CheckInFormDialogProps {
  isOpen: boolean;
  setIsOpen: (open: boolean) => void;
  createGuest: (data: CreateGuestInput) => Promise<void>;
...
const CheckInFormDialog: React.FC<CheckInFormDialogProps> = ({ isOpen, setIsOpen, createGuest, prefill }) => {
  const [formData, setFormData] = useState<CreateGuestInput>(INITIAL_FORM_STATE);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const { toast } = useToast();

  // We now get available rooms from the RoomContext, the correct source of truth
  const { availableRooms, fetchAvailableRooms, loading: roomsLoading } = useRoomContext();

  // Effect to manage form state and fetch rooms when dates change
  useEffect(() => {
    if (isOpen) {
      ...
      // If check-out date is known (e.g., from a reservation), fetch rooms immediately
  ...
  // Fetch available rooms whenever the checkout date changes
  const handleCheckoutDateChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const checkOutDate = e.target.value;
    // Reset room selection when dates change to avoid booking an invalid room
    setFormData(prev => ({ ...prev, checkOutDate, roomNumber: "" }));

    if (checkOutDate && new Date(checkOutDate) > new Date(formData.checkInDate)) {
  ...
  const handleSelectChange = (name: "roomNumber" | "paymentMethod", value: string) => {
    setFormData(prev => ({ ...prev, [name]: value }));
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        title: "Check-in Failed",
        description: err?.response?.data?.message || "Could not create the guest record.",
        variant: "destructive",
      });
    } finally {
      setIsSubmitting(false);
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 pt-4">
          {/* Personal Details */}
          <Input name="fullName" value={formData.fullName} onChange={handleFormChange} placeholder="Full Name" required disabled={isSubmitting} />
          <Input name="address" value={formData.address} onChange={handleFormChange} placeholder="Address" required disabled={isSubmitting} />
          <div className="grid grid-cols-2 gap-4">
            <Input name="phone" value={formData.phone} onChange={handleFormChange} placeholder="Phone Number" required disabled={isSubmitting} />
            <Input name="cnic" value={formData.cnic} onChange={handleFormChange} placeholder="CNIC" required disabled={isSubmitting} />
          </div>
          <Input name="email" type="email" value={formData.email || ''} onChange={handleFormChange} placeholder="Email (Optional)" disabled={isSubmitting} />
          
          {/* Booking Details with Dates */}
          <div className="grid grid-cols-2 gap-4">
            <div>
              <Label htmlFor="checkInDate">Check-in Date</Label>
              <Input id="checkInDate" name="checkInDate" type="date" value={formData.checkInDate} readOnly className="mt-1 bg-gray-100" />
            </div>
            <div>
              <Label htmlFor="checkOutDate">Check-out Date</Label>
              <Input id="checkOutDate" name="checkOutDate" type="date" value={formData.checkOutDate} onChange={handleCheckoutDateChange} className="mt-1" required disabled={isSubmitting} />
            </div>
          </div>
          ...
          <div>
            ...
            <Select name="roomNumber" value={formData.roomNumber} onValueChange={(v) => handleSelectChange("roomNumber", v)} disabled={isSubmitting || !formData.checkOutDate || roomsLoading}>
              <SelectTrigger>
              ...
              <SelectContent>
                {availableRooms.length > 0 ? (
                  ...
                  <div className="px-2 py-4 text-center text-gray-500">
                    {formData.checkOutDate ? "No rooms available for these dates" : "Select a check-out date to see rooms"}
                  </div>
                )}
              </SelectContent>
            </Select>
          ...
          {/* Financial Details */}
          <div className="grid grid-cols-2 gap-4">
            <Input name="additionaldiscount" type="number" value={formData.additionaldiscount || ''} onChange={handleFormChange} placeholder="Additional Discount (Rs)" disabled={isSubmitting} />
            <Select name="paymentMethod" value={formData.paymentMethod} onValueChange={(v) => handleSelectChange("paymentMethod", v)} disabled={isSubmitting}>
          ...
          <div className="flex items-center space-x-2">
            <input type="checkbox" name="applyDiscount" checked={formData.applyDiscount} onChange={handleFormChange} id="applyDiscount" className="h-4 w-4" disabled={isSubmitting} />
            <Label htmlFor="applyDiscount">Apply Standard Discount</Label>
          </div>

          <Button type="submit" className="w-full" disabled={isSubmitting || !formData.roomNumber}>
            {isSubmitting ? "Processing..." : "Submit Check-In"}
          </Button>
        </form>
      </DialogContent>
