...
interface Reservation {
  ...
  startAt: string;
  endAt: string;
  status: "pending" | "reserved" | "cancelled" | "confirmed" | "checked-in" | "checked-out";
  createdAt: string;
  updatedAt: string;
  isPaid?: boolean;
  createdBy?: any;
...
const ReservationDetailsPage: React.FC = () => {
  ...
  const roomDetails = useMemo(() => {
    ...
    if (allRooms && allRooms.length) {
      const guestContextRoom = allRooms.find((room) => room.roomNumber === reservation.roomNumber);
      if (guestContextRoom) return guestContextRoom;
    }

    // Fallback to RoomContext if not found
    if (rooms && rooms.length) {
      return rooms.find((room) => room.roomNumber === reservation.roomNumber) || null;
    }
    ...
    items.map((item) => {
      ...
      return (
        ...
        >
          ...
          <Icon className={`mr-3 h-5 w-5 ${active ? "text-amber-400" : "text-slate-400 group-hover:text-slate-300"}`} />
  ...
  const getRoomCategory = () => {
    ...
    return roomDetails.category || roomDetails.type || roomDetails.roomType || "—";
  };
        ...
        <>
          ...
          >
            <div className="h-20 px-6 flex items-center border-b border-slate-800/50">
              ...
              <button onClick={() => setSidebarOpen(false)} className="lg:hidden ml-auto p-1.5 rounded-lg hover:bg-slate-800/50 transition-colors">
      ...
      <div className="flex-1 w-full h-screen overflow-y-auto flex flex-col">
        ...
        <div className="container mx-auto p-4 md:p-6 lg:p-8 flex-grow">
          <div className="mb-6 flex flex-col sm:flex-row sm:items-center gap-4">
            <Button variant="outline" size="sm" className="w-fit" onClick={() => navigate("/reservation")}>
          ...
          {isLoading ? (
            ...
            <Alert variant="destructive">
              <AlertDescription>Failed to load reservation details. Please try again.</AlertDescription>
            </Alert>
          ) : !reservation ? (
            <Alert>
              <AlertDescription>Reservation not found. It may have been deleted.</AlertDescription>
            </Alert>
          ) : (
                  ...
                  <div className="flex justify-between items-center gap-3">
                    <div>
                      <CardTitle className="text-xl">
                        Reservation - {reservation.fullName} - {isPopulatedRoom(reservation.room) ? reservation.room.roomNumber : reservation.roomNumber}
                      </CardTitle>
                      <CardDescription>Created on {format(new Date(reservation.createdAt), "PPP")}</CardDescription>
                    </div>
                    <div className="ml-auto">{getStatusBadge(reservation.status)}</div>

                    {/* HYBRID navigate: URL param + state prefill */}
                    <Button
                      variant="outline"
                      onClick={() => {
                        const roomNum = typeof reservation.room === "object"
  ? reservation.room.roomNumber
  : reservation.roomNumber;
                        const days = Math.max(
                          1,
                          differenceInCalendarDays(parseISO(reservation.endAt), parseISO(reservation.startAt))
                        );
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        ...
                        <p className="font-medium">
                          Room {isPopulatedRoom(reservation.room) ? reservation.room.roomNumber : reservation.roomNumber}
                        </p>
                      </div>
                      <div>
                      ...
                      <div>
                        ...
                        <p className="font-medium">
                          {reservationDetails?.roomRate ? `Rs. ${reservationDetails.roomRate.toLocaleString()}` : "—"}
                        </p>
                      </div>
                    </div>
                  </div>
...
//     if (roomDetails) {
//       // Handle different property names for rate
//       roomRate = roomDetails.rate || roomDetails.price || 0;
//       totalPrice = roomRate * days;
//     }
...
//   // Get room category with fallbacks for different naming conventions
//   const getRoomCategory = () => {
//     if (!roomDetails) return "—";
//     return roomDetails.category || roomDetails.type || roomDetails.roomType || "—";
//   };
...
//                   <div className="flex justify-between items-center">
//                     <div>
//                       <CardTitle className="text-xl">Reservation - {reservation.fullName} - {reservation.room.roomNumber}</CardTitle>
//                       <CardDescription>Created on {format(new Date(reservation.createdAt), "PPP")}</CardDescription>
//                     </div>
//                     <div>
...
//                       <div>
//                         <p className="text-sm text-gray-500">Room Number</p>
//                         <p className="font-medium">Room {reservation.room.roomNumber}</p>
//                       </div>
//                       <div>
//                         <p className="text-sm text-gray-500">Room Type</p>
//                         <p className="font-medium">{getRoomCategory()}</p>
